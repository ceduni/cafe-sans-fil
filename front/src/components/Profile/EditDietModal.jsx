...
const EditDietModal = ({ isOpen, setIsOpen, diets, diet, setDiets }) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (newDiet.name !== "" && newDiet.forbiddenFoods.length !== 0 && newDiet.forbiddenFoods.includes("") === false) {
            const dietIndex = diets.findIndex((d) => d.name === diet.name);
            const dietsCopy = [...diets];
            dietsCopy[dietIndex] = newDiet;
            setDiets(dietsCopy);
            setIsOpen(false)
    ...
    const formatIngredients = (ingredientsInput) => {
        ...
        setNewDiet({ ...newDiet, forbiddenFoods: splittedIngrdients.map((ingredient) => ingredient.trim().toLowerCase()) });
    };
        ...
        <form className="p-4 md:p-5">
            <div className="grid gap-4 mb-4 grid-cols-2">
                ...
                >
                    ...
                    defaultValue={diet.forbiddenFoods.join(", ")}
