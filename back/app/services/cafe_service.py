...
class CafeService:
    ...
    async def list_menu_items(**query_params) -> List[MenuItem]:
        ...
        pipeline = [
            ...
            {
                "$sort": {
                    f"menu_items.{sort_by[1:] if sort_by[0] == '-' else sort_by}": (
    ...
    async def update_menu_item(
        cafe_slug: str, item_slug: str, item_data: MenuItemUpdate
    ):
        cafe = await CafeService.retrieve_cafe(cafe_slug)
        if cafe:
    ...
    async def update_staff_member(
        cafe_slug: str, username: str, staff_data: StaffUpdate
    ):
        cafe = await CafeService.retrieve_cafe(cafe_slug)
        if cafe and hasattr(cafe, "staff"):
            for member in cafe.staff:
    ...
    async def search_cafes_and_items(query: str, **filters):
        ...
        if filter_target == "item" or filter_target is None:
            ...
            if query:
                item_query["menu_items"]["$elemMatch"].update(
                    {"$or": [{"name": regex_pattern}, {"description": regex_pattern}]}
                )
            matching_cafes_temp = await Cafe.find(item_query).to_list()
            matching_items = [
                item for cafe in matching_cafes_temp for item in cafe.menu_items
            ]
            if sort:
    ...
    @staticmethod
    async def is_authorized_for_cafe_action_by_id(
    ...
    @staticmethod
    async def is_authorized_for_cafe_action_by_slug(
        cafe_slug: str, current_user: User, required_roles: List[Role]
    ):
        cafe = await Cafe.find_one(
            {"$or": [{"slug": cafe_slug}, {"previous_slugs": cafe_slug}]}
        )
        if not cafe:
            raise ValueError("Cafe not found")
