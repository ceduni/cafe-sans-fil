...
describe("GET /api/v1/orders/productName", () => {
  it("should display the number of sales for a product", async () => {
    const response = await request(server.App).get("/api/v1/orders/Barre Kirkland");
    expect(response.status).toBe(200);
    ...
    .send(
      {
        ...
        "cafe_name": "Pill Pub",
        "cafe_slug": "pill-pub",
        "cafe_image_url": "https://imagedelivery.net/70kpvhLybTnH06xFu8o2DQ/db42a0f6-6e3c-4507-eb1",
        "user_username": "7802085",
        "items": [
          {
            "item_name": "Barre de chocolat",
            "item_slug": "barre-de-chocolat",
            "item_image_url": "https://tsel1.mm.bing.net/th?id=OIP.9LxtNeX56JCCLca0HKfBAgHaHa&pid=Api",
            "quantity": 1,
            "item_price": 1.5,
            "options": []
          },
          {
            "item_name": "Brookside",
            "item_slug": "brookside",
            "item_image_url": "https://external-content.duckduckgo.com/iu/?u=https%3A%2F%2F.bp.blogs.com",
            "quantity": 3,
            "item_price": 0.5,
            "options": []
          }
