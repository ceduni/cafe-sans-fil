...
def test_create_menu_item_success(client, list_cafes, cafe_data, auth_login):
    ...
    response = client.post(
        f"/api/cafes/{cafe_slug}/menu", json=menu_item_data, headers=headers
    )
    assert response.status_code == 200
...
def test_update_menu_item_success(client, list_cafes, cafe_data3, auth_login):
    ...
    menu_item_data = cafe_data3
    response = client.put(
        f"/api/cafes/{cafe_slug}/menu/{item_slug}", json=menu_item_data, headers=headers
    )
    assert response.status_code == 200
...
def test_update_menu_item_unauthorized(client, list_cafes, cafe_data3):
    ...
    menu_item_data = cafe_data3
    response = client.put(
        f"/api/cafes/{cafe_slug}/menu/{item_slug}", json=menu_item_data
    )
    assert response.status_code == 401
...
def test_delete_menu_item_success(client, list_cafes, auth_login):
    ...
    response = client.delete(
        f"/api/cafes/{cafe_slug}/menu/{item_slug}", headers=headers
    )
    assert response.status_code == 200
...
def test_create_staff_member_success(client, list_cafes, auth_login):
    ...
    cafe_slug = list_cafes[0]["slug"]
    staff_data = {"username": "TestStaff", "role": "Bénévole"}
    tokens = auth_login
    headers = {"Authorization": f"Bearer {tokens['access_token']}"}
    response = client.post(
        f"/api/cafes/{cafe_slug}/staff", json=staff_data, headers=headers
    )
    assert response.status_code == 200
...
def test_update_staff_member_success(client, list_cafes, auth_login):
    ...
    headers = {"Authorization": f"Bearer {tokens['access_token']}"}
    response = client.put(
        f"/api/cafes/{cafe_slug}/staff/{username}", json=staff_data, headers=headers
    )
    assert response.status_code == 200
...
def test_delete_staff_member_success(client, list_cafes, auth_login):
    ...
    headers = {"Authorization": f"Bearer {tokens['access_token']}"}
    response = client.delete(
        f"/api/cafes/{cafe_slug}/staff/{username}", headers=headers
    )
    assert response.status_code == 200
